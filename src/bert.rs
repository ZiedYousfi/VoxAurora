...
fn get_model() -> &'static SentenceEmbeddingsModel {
    SENTENCE_EMBEDDINGS_MODEL.with(|model_cell| {
        ...
        // This is safe because the RefCell lives for the entire program duration
        // and we never remove the model once initialized
        unsafe { &*(model_ref.as_ref().unwrap() as *const SentenceEmbeddingsModel) }
    })
}

pub fn encode_sentence(sentence: &str) -> Result<Vec<f32>, Box<dyn std::error::Error + Send + Sync>> {
    let model = get_model();
    let output = model
        .encode(&[sentence])
...
pub fn find_best_match<T: AsRef<str> + Clone>(
    ...
    for candidate in candidates {
        ...
        println!("Comparing to candidate '{}': similarity = {:.3}", candidate_str, similarity);
