...
use crate::bert::{encode_sentence, cosine_similarity};
...
fn is_wake_word_present_sync(
    ...
    for token_index in 0..num_tokens {
        ...
        if token_prob <= TOKEN_PROB_THRESHOLD {
            continue;
        }
        ...
        let token_embedding = match encode_sentence(token_clean) {
            Ok(emb) => emb,
            Err(e) => {
                eprintln!("Erreur lors de l'encodage du token '{}': {}", token_clean, e);
                continue;
            }
        };
        ...
        for (i, &wake_word) in WAKE_VARIANTS.iter().enumerate() {
            ...
            let similarity = cosine_similarity(&token_embedding, candidate_embedding);
            println!(
                "Comparaison token '{}' avec '{}' : similarité = {:.3}",
                token_clean, wake_word, similarity
            );
            if similarity > EMBEDDING_SIMILARITY_THRESHOLD {
                println!(
                    "✨ Wake word embedding match: '{}' to '{}' (similarité: {:.3}, p = {:.3})",
                    token_clean, wake_word, similarity, token_prob
                );
                return Ok(true);
            }
        }
